...
void navigateToPool(boolean isTop){
  ...
  correctNavigate(1500,1700);
  correctNavigate(1940,1500);
...
void correctNavigate(int xdest, int ydest) { //Correct right navigate
  ...
  rf.sendMessage("now navigating to: ");
  rf.sendMessage(xdest);
  rf.sendMessage(", ");
  rf.sendMessage(ydest);
  rf.sendMessage("\n");

  while (abs(x - xdest) > 100 || abs(y - ydest) > 100) {
